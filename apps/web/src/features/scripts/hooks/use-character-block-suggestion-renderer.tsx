...
import type { iCharacterBlockSuggestion } from '../extensions/character-block-suggestion';

interface iCharacterBlockSuggestionRenderState {
  floatingElement: HTMLElement | null;
  root: Root | null;
  cleanup: (() => void) | null;
}

export interface iUseCharacterBlockSuggestionRendererOptions {
  /** Suggestions to display */
  suggestions: iCharacterBlockSuggestion[];
  /** Whether suggestions are loading */
  isLoading: boolean;
  /** Called when search query changes */
  onQueryChange: (query: string) => void;
  /** Called when a character is selected */
  onSelect: (character: iCharacterBlockSuggestion) => void;
...
export type UseCharacterBlockSuggestionRendererOptions = iUseCharacterBlockSuggestionRendererOptions;

interface iCharacterBlockSuggestionRendererConfig extends Partial<SuggestionOptions<iCharacterBlockSuggestion>> {
  items?: (props: { query: string }) => iCharacterBlockSuggestion[] | Promise<iCharacterBlockSuggestion[]>;
  render?: () => {
    onStart: (props: SuggestionProps<iCharacterBlockSuggestion>) => void;
    onUpdate: (props: SuggestionProps<iCharacterBlockSuggestion>) => void;
...
/**
 * Creates a suggestion configuration for the CharacterBlockSuggestion TipTap extension.
 * Renders the EntityAutocomplete component for character selection in Character blocks.
 *
 * @example
 * ```tsx
 * const { suggestions, isLoading, setQuery } = useCharacterSuggestions({ seriesId });
 *
 * const suggestionConfig = useCharacterBlockSuggestionRenderer({
 *   suggestions,
 *   isLoading,
 ...
 * });
 *
 * // Use with CharacterBlockSuggestion extension
 * CharacterBlockSuggestion.configure({
 *   suggestion: suggestionConfig,
 * });
 * ```
 */
export function useCharacterBlockSuggestionRenderer({
  suggestions,
  isLoading,
  onQueryChange,
  onSelect,
...
}: iUseCharacterBlockSuggestionRendererOptions): iCharacterBlockSuggestionRendererConfig {
  // Keep refs to mutable state
  const stateRef = useRef<iCharacterBlockSuggestionRenderState>({
    floatingElement: null,
    root: null,
    cleanup: null,
  });
  const currentPropsRef = useRef<SuggestionProps<iCharacterBlockSuggestion> | null>(null);
  ...
  const renderConfig = useCallback(() => {
    ...
    return {
      onStart: (props: SuggestionProps<iCharacterBlockSuggestion>) => {
        currentPropsRef.current = props;
        onQueryChangeRef.current(props.query);

        // Create floating container element
        const floatingEl = document.createElement('div');
        floatingEl.className = 'character-block-autocomplete-floating';
        ...
        container.className = 'character-block-autocomplete-container';
        ...
        // Filter out loading placeholder
      ...
      onUpdate: (props: SuggestionProps<iCharacterBlockSuggestion>) => {
        currentPropsRef.current = props;
        onQueryChangeRef.current(props.query);
  ...
  const suggestionConfig = useMemo<iCharacterBlockSuggestionRendererConfig>(
    () => ({
